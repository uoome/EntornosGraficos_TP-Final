...
class Cart
{
    ...
    public function insert($item = array())
    {
        if (!is_array($item) or count($item) === 0) {
            return FALSE; // Si no es array o viene vacio
        } else {
            if (!isset($item['id'], $item['nombre'], $item['precio'], $item['qty'])) {
                die("Datos erroneos" . print_r($item));
                ...
                if ($item['qty'] == 0) {
                    return FALSE; // Error en parseo, setear 0
                }
                // prep the price
                $item['precio'] = (float) $item['precio'];
                // create a unique identifier for the item being inserted into the cart
                $rowid = md5($item['id_zapatilla']);
    ...
    protected function save_cart()
    {
        ...
        foreach ($this->cart_contents as $key => $val) {
            ...
            if (!is_array($val) or !isset($val['precio'], $val['qty'])) {
                // if (!is_array($val) or !isset($val['precio'])) {
                continue;
            }
    ...
    public function remove($row_id)
    {
        ...
        // var_dump($row_id);
        // echo "Item: <br>";
        // var_dump($this->cart_contents[$row_id]);
        unset($this->cart_contents[$row_id]);
        // echo "Carro post delete: <br>";
        // die(var_dump($this->cart_contents));
